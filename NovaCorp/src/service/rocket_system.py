...
class RocketSystem: 
    ...
    def __init__(self):
        self._navigation_system = NavigationSystem()
        self._communication_system = CommunicationSystem()
        self._weather_system = WeatherSystem()
        self._fuel_system = FuelSystem()

    def launch_rocket(self, lat: float, long: float, fuel: float):
        self._weather_system.analyze()
        time.sleep(1)
        fuel_check = self._fuel_system.check()
        print(f'The fuel is correct: {fuel_check}')
        time.sleep(1)
        self._fuel_system.load(fuel)
        trajectory = self._navigation_system.calculate_trajectory(lat, long)
        time.sleep(1)
        print(f'Trajectory calculated to {trajectory}')
        self._navigation_system.set_coordinates(lat, long)
        time.sleep(1)
        self._communication_system.connect_to_central()
        time.sleep(4)
